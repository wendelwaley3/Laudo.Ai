...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        map: null,
        layers: {
            lotes: L.featureGroup(),
            app: L.featureGroup(),
            poligonais: L.featureGroup(),
            risco: L.featureGroup()
        },
        allLotes: [],
        allRisco: [],
        nucleusSet: new Set(),
    ...
    function initMap() {
        ...
        const osmLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 20, attribution: '&copy; OpenStreetMap' }).addTo(state.map);
    ...
    function initUpload() {
        ...
        fileInput.addEventListener('change', () => {
            fileListEl.innerHTML = Array.from(fileInput.files).map(f => `<li>${f.name}</li>`).join('') || '';
        });
